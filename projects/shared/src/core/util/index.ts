export function deepClone<T>(fromObject: T): T {
...
export async function delay(minDelay: number, maxDelay?: number): Promise<void> {
  const delay = maxDelay && maxDelay >= minDelay ? Math.floor(Math.random() * maxDelay + minDelay) : minDelay;
...
export async function retryUntil<T = boolean>(func: () => Promise<T>, check?: (result: T) => boolean, maximumAttempts = 0, delay = 20): Promise<T> {
  let attempt = 0;

  const expect = check ?? identity;

  const retry = (resolve: (result: T) => void, reject: (error: Error) => void) => {
    func().then(result => {
      if (expect(result)) {
        resolve(result);
      } else if (maximumAttempts > 0 && ++attempt >= maximumAttempts) {
...
export async function waitUntil(func: () => boolean | Promise<boolean>, timeout = 0): Promise<void> {
  const funcPromise = func() instanceof Promise ? func as () => Promise<boolean> : async () => func();
  const start = Date.now();
  const check = (result: boolean) => {
    if (result) {
      return true;
  ...
  await retryUntil(funcPromise, check);
}

export const nop = () => void (0);

export function utc(date = new Date()): number {
  return date.getTime() - date.getTimezoneOffset() * 60 * 1000;
}

export type FTranslationTemplate<T extends Record<string, string>> = (pairs?: TranslationTemplate<T>) => string;

export type TranslationTemplate<T extends Record<string, string>> = {
  readonly [Key in Exclude<keyof T, 'template'>]?: number | string | { toString(): string };
}

export type TranslationMapping<T extends Record<string, string | Record<string, string>>> = {
  readonly [Key in keyof T]: T[Key] extends Record<string, string> ? FTranslationTemplate<T[Key]> : T[Key];
}

export function i18n<T extends Record<string, string | Record<string, string>>>(pairs: T): TranslationMapping<T> {
  const i18n: Record<string, string | FTranslationTemplate<never>> = {};
  ...
  for (const [key, value] of Object.entries(pairs)) {
    if (typeof value === 'string') {
      ...
      const placeholders = Object.keys(defaults).join(',');
      const code = `const { ${placeholders} } = { ...defaults, ...pairs ?? {} }; return \`${template}\`;`;

      const templateFunc = new Function('defaults', 'pairs', code);
      i18n[key] = (pairs) => templateFunc(defaults, pairs);
    }
  }

  return i18n as TranslationMapping<T>;
}

/**
 * Genrate a random name, docker container like name. First and lastname array list is copied
 * from https://github.com/moby/moby/blob/4cac624fefefb7bb6640f205587d0dc0a9651b27/pkg/namesgenerator/names-generator.go.
 */
...
export function ucfirst(str: string) {
  return str.length === 0 ? str : str[0].toLocaleUpperCase() + str.substring(1);
}

export function getRandomColor(): string {
  return '#' + Array(6)
    .fill('')
    .map(() => Math.floor(Math.random() * 10 + 3).toString(16))
    .join('');
}
